...
window.addEventListener('wheel', (e)=>{
  ...
  if (deltaY > 0){
    Contador++;
    TitleH2Span.forEach(( _ ,i )=>{
      TitleH2Span[i].classList.remove('TitleH2IsVisible')
    ...
    if( Contador <= 0 ){
      TitleH2Span.forEach(( _ ,i )=>{
        TitleH2Span[i].classList.add('TitleH2IsVisible')
        Contador = 0
      })
    }
    console.log(Contador)
  ...
  if( Contador >= 1){
    Muestra.style.transform = `translateX(-${33.333 * (Contador - 1)}%)`;
  }
  if (Contador >= 6){
...
const PhotoGalleryImg = document.querySelectorAll(`.PhotoGallery-img`)
PhotoGalleryImg.forEach(( _ , i )=>{
  PhotoGalleryImg[i].addEventListener('mouseenter', ()=>{
    PhotoGalleryImg.forEach(( _ , i )=>{
      PhotoGalleryImg[i].style.filter = `brightness(40%)`
    })
    PhotoGalleryImg[i].classList.add('PhotoIsActive')
    PhotoGalleryImg[i].style.filter = `brightness(100%)`

  })

})
PhotoGalleryImg.forEach(( _ , i )=>{
PhotoGalleryImg[i].addEventListener('mouseleave', ()=>{
  PhotoGalleryImg.forEach(( _ , i )=>{
    PhotoGalleryImg[i].style.filter = `brightness(100%)`
  })
  PhotoGalleryImg[i].classList.remove('PhotoIsActive')
})
})
