...
console.log(cuadroWrapper);
const cuadroPrimero = document.querySelector(`.Hero-cuadro--Primero`)
console.log(cuadroPrimero)
const Carrusel = document.querySelector(`.Carrusel`)
console.log(Carrusel)
const Header = document.querySelector(`.Header`)
const HeaderTop = document.querySelector(`.Header-top`)
const HeaderBottom = document.querySelector(`.Header-bottom`)
const HeaderTopSvgWrapper=document.querySelector(`.Header-top-svg-wrapper`)
const HeaderBottomUl = document.querySelector(`.Header-bottom-ul`)
...
cuadros.forEach((cuadro) => {
  cuadro.addEventListener('mouseenter', () => {
    isHoveringOverCuadro = true;
  });

  cuadro.addEventListener('mouseleave', () => {
    isHoveringOverCuadro = false;
  });
});


// Guarda el contenido inicial de cuadroPrimero
...
window.addEventListener('wheel', (e) => {
  ...
  if (deltaY > 0) {
    ...
    if (esScrolear < 0) {
      ...
      // Restablece posicion
      cuadros.forEach((cuadro) => {
        cuadro.style.transform = 'translate(0, 0)'; // Reseteo del movimiento
      });
...
cuadros.forEach((_ , i)=>{
  cuadros[i].addEventListener('click', ()=>{
    cuadros.forEach((cuadro) => {
      cuadro.style.transform = 'translate(0, 0)';
    });
...
let observer = new IntersectionObserver((entries)=>{
  entries.forEach((entry)=>{
    ...
    if( isIntersecting){
      target.classList.add('gridIsVisible')
    }
  })
} , options)
GridLi.forEach(( _ , i )=>{
  observer.observe(GridLi[i])
})
...
let galleryobserver = new IntersectionObserver((entries)=>{
  entries.forEach((entry)=>{
    let { isIntersecting , target } = entry
    if( isIntersecting){
      target.classList.add('galleryIsVisible')
    }
  })
} , galleryoptions)
GalleryLi.forEach(( _ , i )=>{
  galleryobserver.observe(GalleryLi[i])
})
// const GalleryAbutton = document.querySelectorAll(`.Gallery-a--button`)
// console.log(GalleryAbutton)
// const GalleryAbuttonBackground = document.querySelectorAll(`.Gallery-a--button-background`)
// const GalleryAbuttonSpan = document.querySelectorAll(`.Gallery-a--button-span`)
...
Button.forEach((button) => {
  ...
  button.addEventListener('mouseenter', () => {
    ...
    buttonSpans.forEach(span => {
      span.classList.add('buttonSpanIsActive');
    });
  });
  ...
  button.addEventListener('mouseleave', () => {
    ...
    buttonSpans.forEach(span => {
      span.classList.remove('buttonSpanIsActive');
    });
  });
});

const ContactCuadro = document.querySelectorAll(`.Contact-cuadro`)
const ContactCuadroPrimero = document.querySelector(`.Contact-cuadro--Primero`)
ContactCuadroPrimero.addEventListener('mouseenter', ()=>{
    ContactCuadro.forEach((_ , i)=>{
      ContactCuadro[i].classList.add('contactCuadroIsActive')
    })
  })
ContactCuadroPrimero.addEventListener('mouseleave', ()=>{
    ContactCuadro.forEach((_ , i)=>{
      ContactCuadro[i].classList.remove('contactCuadroIsActive')
    })
  })
